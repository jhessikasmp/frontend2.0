...
const Emergencia: React.FC = () => {
	...
	const atualizarTudo = async () => {
		...
		const totalYear = entries.reduce((sum: number, e: any) => sum + (e.valor || 0), 0);
		setEntradasAnual(totalYear);
		const total = await getTotalEmergencyEntries();
		setEntradasTotal(total);
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
				<div className="bg-gradient-to-br from-blue-500 to-blue-700 dark:from-blue-800 dark:to-blue-950 p-5 rounded-xl text-white shadow-lg flex flex-col justify-between min-h-[120px] border border-blue-400/20 hover:shadow-xl transition-shadow">
					<div className="flex items-center gap-2 mb-2">
						...
						<span className="text-base font-semibold">Entradas</span>
					</div>
					...
					<span className="text-lg font-bold block">Total: {!showValues ? '•••' : entradasTotal.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</span>
				</div>
				<div className="bg-gradient-to-br from-red-500 to-red-700 dark:from-red-800 dark:to-red-950 p-5 rounded-xl text-white shadow-lg flex flex-col justify-between min-h-[120px] border border-red-400/20 hover:shadow-xl transition-shadow">
					<div className="flex items-center gap-2 mb-2">
						...
						<span className="text-base font-semibold">Total de Despesas</span>
					</div>
					<span className="text-3xl font-bold">{!showValues ? '•••' : `€ ${totalDespesas.toLocaleString('de-DE', { minimumFractionDigits: 2 })}`}</span>
				</div>
					...
					<div className="p-6">
						{despesas.length === 0 ? (
							...
							<div className="space-y-3">
								{despesas.map((d, i) => (
									<div key={d._id || i} className="bg-gradient-to-r from-gray-50 to-red-50 dark:from-gray-700 dark:to-gray-750 rounded-xl p-4 flex justify-between items-center shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
